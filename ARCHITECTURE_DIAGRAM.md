...
```mermaid
flowchart TD
    Start["Start Orchestration"] --> Trigger["Trigger DAG<br/>ra_stage_to_ods_orchestration"]

    Trigger --> LoopStart["Loop: entities × sources"]
    LoopStart --> StageTask["Trigger DAG<br/>ra_&lt;source&gt;_&lt;entity&gt;_stage"]
    StageTask --> StageWait["Wait for completion"]
    StageWait --> ODSTask["Trigger DAG<br/>ra_&lt;source&gt;_&lt;entity&gt;_ods"]
...
DAG identifiers generated by the STAGE and ODS factories
(`airflow/dags/ra_stage_jobs.py`, `airflow/dags/ra_ods_jobs.py`):
...
### 4. Event-Driven Streaming Flow

```mermaid
flowchart LR
    Producer["Producer<br/>(Developer UI / API / external)"] --> Topic["Kafka topic<br/>risk.customer.ingest"]
    Topic --> Consumer["kafka-entity-stream<br/>jobs/kafka_entity_consumer.py"]
    Consumer --> Trigger["Kafka topic<br/>risk.pipeline.trigger"]
    Trigger --> StageDag["DAG ra_kafka_customer_stage<br/>AwaitMessageSensor"]
    StageDag --> StageJob["Spark: run_source_to_ods_step.py<br/>--layer stage --entity customer"]
    StageJob --> OdsDag["DAG ra_kafka_customer_ods"]
    OdsDag --> OdsJob["Spark: run_source_to_ods_step.py<br/>--layer ods --entity customer"]
...
│   ├── ra_common.py           # Shared constants + spark-submit command builders
│   ├── ra_createtables_and_data.py     # Bootstrap: create tables, seed sources
│   ├── ra_stage_jobs.py       # Factory: ra_<source>_<entity>_stage DAGs
│   ├── ra_ods_jobs.py         # Factory: ra_<source>_<entity>_ods DAGs
│   ├── ra_stage_to_ods_orchestration.py  # Stage -> ODS -> risk metrics
│   ├── ra_riskmetrics_eval_ods.py      # Final risk metric evaluation
│   └── ra_kafka_streaming.py  # ra_kafka_customer_stage / ra_kafka_customer_ods
...
│   └── execute_pipeline.py    # Ad-hoc YAML pipeline runner
...
├── api/                       # FastAPI links portal + operational endpoints
