...
flowchart TB
    subgraph "User Access Layer"
        ...
        LinksAPI["Operations API + links portal<br/>FastAPI :8000"]
...
The eight task groups are independent, so STAGE and ODS work for different
source/entity pairs overlaps. Concurrency is bounded by the `spark_submit`
Airflow pool (`SPARK_SUBMIT_POOL_SLOTS`, default 2) rather than by the DAG
structure, because every leaf task starts a local Spark driver in the Airflow
container. The waits are deferred, so the `airflow-triggerer` service must run.
...
│   └── ra_kafka_streaming.py  # Factory: ra_kafka_<entity>_stage / _ods DAGs
